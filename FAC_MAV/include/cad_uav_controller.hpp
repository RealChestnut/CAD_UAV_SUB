...
double tau_y_limit = 0.2; // yaw torque limit
...
double torque_dob_fc = 1.5; //origin :: 3.0
double dhat_tau_r = 0;
double dhat_tau_p = 0;
double dhat_tau_y = 0;
...
double x_th3 = 0;
double x_th4 = 0;
double th_cut_off_freq = 100.0; // origin :: 15
...
void shape_detector()
{
  ...
  if( /*(button_cnt==(50*button_limit)) &&*/ main_agent){ // we define that this state is disassembled
  	  //mono_flight = true;
  	  module_num=1;
...
void PWM_signal_Generator()
{
  ...
  if(!main_agent && !mono_flight){ /// 서브드론의 경우 Sbus[3]를 다이나믹셀 position desired input으로 사용
	...
	if(is_Mani == true){swap_dynamixel_angle = 0.02;
        if(servo_trailer > 900){swap_dynamixel_angle = 0.0;}}
        if(is_Mani == false){
                swap_dynamixel_angle = -0.02;
...
Eigen::Matrix3d R_z;
Eigen::Matrix3d R_x;
Eigen::Vector3d posi_vec3d;
Eigen::Vector3d rot_vec3d;

Eigen::Vector3d main_vec3d;
...
void main_pose_data_Callback(const std_msgs::Float32MultiArray& msg){
	...
	*/
	...
	main_vec3d << main_position_opti_new.x, main_position_opti_new.y, main_position_opti_new.z;
	main_vec3d = R_z*R_x*main_vec3d;
*/
	/*
...
void sub_pose_data_Callback(const std_msgs::Float32MultiArray& msg){
	...
	/*
	posi_vec3d << position_from_t265.x,position_from_t265.y,position_from_t265.z-0.18611;
        rot_vec3d << sub_attitude_opti.x,sub_attitude_opti.y,sub_attitude_opti.z;
	...
	posi_vec3d = R_z*R_x*posi_vec3d;
        rot_vec3d = R_z*R_x*rot_vec3d;



        position_from_t265.x = posi_vec3d(0);
        position_from_t265.y = posi_vec3d(1);
        position_from_t265.z = posi_vec3d(2);

        sub_attitude_opti.x = rot_vec3d(0);
        sub_attitude_opti.y = rot_vec3d(1);
        sub_attitude_opti.z = rot_vec3d(2);
	*/
...
void kalman_Filtering(){
	...
	z << position_from_t265.x, position_from_t265.y, position_from_t265.z;
