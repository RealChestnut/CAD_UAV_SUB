...
{  
    ...
    tf::Quaternion quat;
    Eigen::AngleAxisd yawAngle_body(yaw_pre, Eigen::Vector3d::UnitZ()); //optitrack 기준 :: yaw --> Y_axis
    Eigen::AngleAxisd pitchAngle_body(pitch_pre, Eigen::Vector3d::UnitY()); //optitrack 기준 :: pitch --> X axis 
    Eigen::AngleAxisd rollAngle_body(roll_pre, Eigen::Vector3d::UnitX()); // optitrack 기준 :: roll --> Z axis
    ...
    tf2::Matrix3x3(quat_body).getRPY(yaw,roll,pitch); //yaw.roll,pitch
...
void SUB_PoseCallback(const geometry_msgs::PoseStamped &msg)
{
    ...
    tf2::Quaternion quat_pre(msg.pose.orientation.x,msg.pose.orientation.y,msg.pose.orientation.z,msg.pose.orientation.w);
    tf2::Matrix3x3 pre_mat = tf2::Matrix3x3(quat_pre); // quat_pre를 matrix 3x3에 mapping함
    pre_mat.getRPY(roll_pre,pitch_pre,yaw_pre);
    //ROS_INFO("roll ; %lf|pitch ; %lf|yaw : %lf",Roll_main,Pitch_main,Yaw_main);
    ...
    X_sub = msg.pose.position.y;//posi_cut_off_freq*X_position(0);
    Y_sub = msg.pose.position.z;//posi_cut_off_freq*X_position(1);
    Z_sub = msg.pose.position.x;//posi_cut_off_freq*X_position(2);
