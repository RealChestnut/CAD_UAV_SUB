...
double trailer_current = 0.0;
int trailer_cnt = 0;
bool trailer_ctrl_ = false;

void jointstate_Callback(const sensor_msgs::JointState& msg)
{
  trailer_current = msg.effort[4];
}
int switch_data;
int x_s_dot = 0;
int x_s = 0;
...
int main (int argc, char** argv){
    ...
    ros::Subscriber dynamixel_state = nh.subscribe("joint_states",1,jointstate_Callback, ros::TransportHints().tcpNoDelay());
	...
	//ser.flush();}
	   ...
	   if(is_Mani){
		...
		if((trailer_current > 900) && trailer_cnt<20){
			trailer_cnt++;
		...
		if(trailer_cnt==20){
			...
			ForToString = to_string(trailer_ctrl_);

           		ser.write(ForToString);
		}
